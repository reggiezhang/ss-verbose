...
const char* host_match_to_str(int host_match) {
    ...
    } else if (get_acl_mode() == BLACK_LIST) {
            ...
            if (acl
                ...
                */
                ...
                if (bypass) {
                    if (verbose) {
                        if (sni_detected || atyp == 3)
                            LOGI("bypass(%s): %s:%s", host_match_to_str(host_match), host, port);
                        else if (atyp == 1)
                            LOGI("bypass(%s): %s:%s", host_match_to_str(host_match), ip, port);
                        else if (atyp == 4)
                            LOGI("bypass(%s): [%s]:%s", host_match_to_str(host_match), ip, port);
                    }
#ifndef __ANDROID__
                    ...
                    if (verbose) {
                        if (sni_detected || atyp == 3)
                            LOGI("proxying(%s): %s:%s", host_match_to_str(host_match), host, port);
                        else if (atyp == 1)
                            LOGI("proxying(%s): %s:%s", host_match_to_str(host_match), ip, port);
                        else if (atyp == 4)
                            LOGI("proxying(%s): [%s]:%s", host_match_to_str(host_match), ip, port);
                    }
                }
...
main(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, "f:s:p:l:k:t:m:i:c:b:a:n:huUvV6A",
                            long_options, NULL)) != -1) {
#else
    while ((c = getopt_long(argc, argv, "f:s:p:l:k:t:m:i:c:b:a:n:huUvw6A",
                            long_options, NULL)) != -1) {
#endif
        switch (c) {
